...
### Load stack and clim
load('./outputs/RG_realClim_cc4_rep_1_GenSA_rf_0_ts_10_8000.rdata')
...
datClim <- subset(datClim,step=="20"|step=="0"|step=="100"|step=="200"|step=="500")
...
prop_TM$step <- factor(prop_TM$step,levels=rev(as.numeric(levels(prop_TM$step))*5),labels=rev(as.numeric(levels(prop_TM$step))*5))
#prop_TM <- subset(prop_TM,step=="20"|step=="0"|step=="100"|step=="200"|step=="500")
...
scale_fill_manual(values = rev(brewer.pal(5,"YlOrBr")),name="Years: ")+
geom_vline(xintercept=15, colour="grey25", linetype = "longdash") +
geom_text(aes(x=15, label="Montréal", y=0.25), colour="grey25", hjust = -0.3, text=element_text(size=40))+
geom_vline(xintercept=500, colour="grey25", linetype = "longdash") +
geom_text(aes(x=500, label="Chibougamau", y=0.25), colour="grey25",  hjust = 1.2, text=element_text(size=40))+
scale_x_continuous(expand=c(0,0),breaks=c(seq(0,500,40)))+
